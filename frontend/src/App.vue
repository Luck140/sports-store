<template>
  <div id="app">
    ...
    <div class="app-layout" v-if="userStore.user">
      <el-menu :default-active="currentRoute" router class="sidebar">
...
.sidebar { width:200px; min-height:100vh; overflow-y:auto; flex-shrink:0; }
.sidebar .logo { height:60px; line-height:60px; text-align:center; color:#fff; font-size:18px; font-weight:bold; background:var(--color-dark-bg-2); }
.main-area { flex:1; display:flex; flex-direction:column; min-width:0; }
.topbar { height:50px; background:#fff; border-bottom:1px solid var(--color-border); display:flex; align-items:center; justify-content:flex-end; padding:0 20px; gap:16px; font-size:14px; }
.content { flex:1; padding:20px; background:var(--color-bg); overflow-y:auto; }
.guest-layout { height:100%; }
.guest-nav { display:flex; justify-content:space-between; align-items:center; padding:0 40px; height:60px; background:var(--color-surface); border-bottom:1px solid var(--color-border-lighter); }
.guest-nav .logo-text { font-size:20px; font-weight:bold; color:var(--color-text); }
.guest-content { height:calc(100% - 60px); background:var(--color-bg); }
@media (max-width:768px) { .sidebar { width:160px; } .content { padding:12px; } .guest-nav { padding:0 16px; } }
@media (max-width:480px) { .app-layout { flex-direction:column; } .sidebar { width:100%; min-height:auto; max-height:50vh; } .topbar { padding:0 12px; gap:8px; } }
</style>