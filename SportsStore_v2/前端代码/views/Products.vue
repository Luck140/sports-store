<template>
  <div>
    <div class="breadcrumb">
      ...
      <span>{{ currentCategory || '全部商品' }}</span>
    </div>
    <el-card class="filter-card">
      <el-row :gutter="10" align="middle">
        <el-col :xs="24" :sm="6">
          <el-input v-model="keyword" placeholder="搜索商品名称或描述" :prefix-icon="SearchIcon" clearable size="default" @keyup.enter="doSearch" @clear="doSearch" />
        </el-col>
        <el-col :xs="12" :sm="4">
          <el-select v-model="currentCategory" placeholder="全部分类" clearable size="default" style="width:100%" @change="onCategoryChange">
            <el-option v-for="c in cats" :key="c" :label="c" :value="c" />
          </el-select>
        </el-col>
        <el-col :xs="12" :sm="4">
          <el-select v-model="currentSort" placeholder="排序" size="default" style="width:100%" @change="doSearch">
        ...
        <el-col :xs="24" :sm="6">
          <div class="price-range">
            <el-input v-model="minPrice" placeholder="最低价" size="default" style="width:90px" clearable @change="doSearch" @clear="doSearch" />
            <span class="range-sep">—</span>
            <el-input v-model="maxPrice" placeholder="最高价" size="default" style="width:90px" clearable @change="doSearch" @clear="doSearch" />
          </div>
        </el-col>
        <el-col :xs="0" :sm="4" style="text-align:right">
          <span v-if="total >= 0" class="total-hint">共 {{ total }} 件</span>
          <el-button v-if="userStore.isAdmin" type="primary" size="small" @click="openAdd" style="margin-left:8px">+ 添加商品</el-button>
        </el-col>
      </el-row>
    </el-card>
      ...
      <el-row :gutter="14" v-else>
        <el-col :xs="24" :sm="12" :md="8" :lg="6" v-for="p in list" :key="p.product_id">
          <div class="pcard" @click="$router.push(`/products/${p.product_id}`)">
            <div class="pcard-icon">{{ getProductIcon(p) }}</div>
            <div class="pcard-name" :title="p.product_name">{{ p.product_name }}</div>
            <div class="pcard-cat">{{ p.category }}<span v-if="p.sales_count"> · 已售 {{ p.sales_count }}</span></div>
            <div class="pcard-row">
              <span class="pcard-price">¥{{ p.unit_price }}</span>
              <span class="pcard-stock" :style="{ color: stockColor(p) }">{{ stockText(p) }}</span>
            </div>
            <el-button
              v-if="!userStore.isAdmin && p.stock_quantity > 0"
              type="primary" size="small" @click.stop="addCart(p.product_id)"
              style="width:100%;margin-top:10px">加入购物车</el-button>
            <el-button v-else-if="!userStore.user && p.stock_quantity > 0"
              size="small" @click.stop="$router.push('/login')" style="width:100%;margin-top:10px">登录后购买</el-button>
            <el-button v-else-if="userStore.isAdmin"
              size="small" @click.stop="openEdit(p)" style="width:100%;margin-top:10px">编辑</el-button>
            <el-button v-else disabled size="small" style="width:100%;margin-top:10px">暂时缺货</el-button>
          </div>
        </el-col>
      </el-row>
    ...
    <el-dialog v-model="dlg" :title="editProd ? '编辑商品' : '添加商品'" width="480px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="库存"><el-input-number v-model="form.stock_quantity" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="预警值"><el-input-number v-model="form.min_stock_threshold" :min="0" style="width:100%" /></el-form-item>
...
const minPrice = ref(''); const maxPrice = ref(''); const loading = ref(false)
const total = ref(0); const page = ref(1); const pageSize = 12; const totalPages = ref(1)
const cats = ['球类', '服装', '器材', '鞋类', '配件']
const icons = ['🏀', '⚽', '🏸', '👕', '⚾', '🏐', '🏓', '🎽', '🩳', '🧢']
...
const stockColor = (p) => p.stock_quantity > 10 ? 'var(--ss-success)' : p.stock_quantity > 0 ? 'var(--ss-warning)' : 'var(--ss-danger)'
const stockText = (p) => p.stock_quantity > 10 ? '有货' : p.stock_quantity > 0 ? `仅剩 ${p.stock_quantity} 件` : '缺货'
...
const load = async (pg) => {
  ...
  try {
    ...
    const res = await axios.get(keyword.value ? '/api/products/search/' : '/api/products/', { params })
    const d = res.data
    if (d && d.items) { list.value = d.items; total.value = d.total || 0; totalPages.value = d.total_pages || 1 }
    else { list.value = []; total.value = 0 }
  } catch { list.value = []; total.value = 0 }
  finally { loading.value = false }
}

const onCategoryChange = (cat) => { doSearch() }
let timer = null
const doSearch = () => { page.value = 1; clearTimeout(timer); timer = setTimeout(() => load(1), 300) }
const goPage = (p) => { page.value = p; load(p) }
const resetAll = () => { keyword.value = ''; currentCategory.value = ''; currentSort.value = ''; minPrice.value = ''; maxPrice.value = ''; page.value = 1; load(1) }
...
.range-sep { color: var(--ss-text-dim); }
.total-hint { font-size: 12px; color: var(--ss-text-muted); }
.empty-state { padding: 80px 0; text-align: center; }
.empty-icon { font-size: 48px; margin: 0 0 8px; }
.empty-text { color: var(--ss-text-muted); margin-bottom: 12px; }
.pcard {
  background: var(--ss-surface); border: 1px solid var(--ss-border-light); border-radius: var(--ss-radius-sm);
  padding: 16px; cursor: pointer; transition: all var(--ss-transition-base); margin-bottom: 14px; text-align: center;
}
.pcard:hover { border-color: var(--ss-primary); box-shadow: var(--ss-shadow-md); transform: translateY(-1px); }
.pcard-icon { font-size: 44px; padding: 8px 0; }
.pcard-name { font-size: 14px; font-weight: 600; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.pcard-cat { font-size: 12px; color: var(--ss-text-muted); margin-bottom: 8px; }
.pcard-row { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 4px; }
.pcard-price { font-size: 18px; font-weight: 700; color: var(--ss-accent); }
.pcard-stock { font-size: 11px; }
</style>
